...
def generate(player, space_objects, canv):
    ...
    for i in range(1, quantity_obj + 1):
        ...
        x1 = round(uniform(player.x + window_width / 2,
                           player.x + window_width / 2 + 300))
        x2 = round(uniform(player.x - window_width / 2,
                           player.x - window_width / 2 - 300))
        print(x1, x2)
        x = choice([x1, x2])
        y1 = round(uniform(player.y + window_height / 2,
                           player.y + window_height / 2 + 300))
        y2 = round(uniform(player.y - window_height / 2,
                           player.y - window_height / 2 - 300))
        y = choice([y1, y2])
        vx = uniform(-0.1, 0.1)
        vy = uniform(-0.1, 0.1)
